...
内置 412/-352 风控重试机制，遇到风控会自动等待后继续。
...
### 清理已注销关注 (`remove_none_subscribe.py`)

自动扫描关注列表，取消关注已注销的用户。内置白名单机制，白名单中的账号不会被取关（默认包含「哔哩哔哩番剧出差」）。

```bash
python remove_none_subscribe.py
```

### 关注哔哩哔哩番剧出差 (`关注哔哩哔哩番剧出差.py`)

自动关注 [哔哩哔哩番剧出差](https://space.bilibili.com/11783021)（uid: 11783021）。

```bash
python 关注哔哩哔哩番剧出差.py
```

## 可能问题

批处理可能触发 412 安全风控策略，脚本已内置重试等待机制。